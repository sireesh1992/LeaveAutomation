...
<table border="1"  style="width: 100%">
	...
	<th>Button</th>
		...
		<s:form theme="simple" action="updateLeaves.action">
			<tr>
				<td><s:property value="leave_id"/><s:hidden name="leave_id" /></td>
				<td><s:property value="soeid_emp"/><s:hidden name="soeid_emp" /></td>
				<td><s:property value="num_days" /><s:hidden name="num_days" /></td>
...
background: linear-gradient(to bottom,  rgba(0,0,0,0),rgba(20,170,255,1));
}

#tabs>ul>li:hover {
	background: linear-gradient(to bottom,  rgba(0,0,0,0),rgba(255,170,0,1));
	cursor: pointer;
	color: black;
}
