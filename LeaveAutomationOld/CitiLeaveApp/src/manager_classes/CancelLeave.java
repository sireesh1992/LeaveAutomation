...
public class CancelLeave {
	...
	{
		DAL_2.updateStatus(leave_id, num_days, category, soeid_emp, A_Rbut);
		//System.out.println(leave_id+"\t"+ num_days+"\t"+ category+"\t"+ soeid_emp+"\t"+ A_Rbut);
		System.out.println(leave_id);
		if(category.equals("Casual"))
		{
			System.out.println(num_days+" "+soeid_emp);
			DAL_1.dec_cl(num_days, soeid_emp);
		}
		
		else if(category.equals("Sick"))
		{
			DAL_1.dec_sl(num_days, soeid_emp);
				
		}
		
		else if(category.equals("Mandatory"))
		{
				DAL_1.dec_ml(num_days, soeid_emp);
		}
		
		else if(category.equals("Paternity/Maternity"))
		{
			DAL_1.dec_pml(num_days, soeid_emp);
		}
